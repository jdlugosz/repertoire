...
template <typename T>
class callback_thunk : private closure_stdcall_to_thiscall {
   static void sizecheck(char (*p)[4]) {}
   static void* helper (int (T::*member)(int));
public:
   callback_thunk (T* object, int (T::*member)(int)) : closure_stdcall_to_thiscall (object, helper(member)) {}
   callback_t* callptr() const { return reinterpret_cast<callback_t*>(this); }
   };


template <typename T>
void* callback_thunk<T>::helper (int (T::*member)(int))
 {
 union {
    int (T::*p)(int);
    void* function;
...
int main()
 ...
 C object (99);
 callback_thunk<C> thunk (&object, &C::callback);
